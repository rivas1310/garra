...
function ProductCardContent({ product, layout = 'grid' }: ProductCardProps) {
  ...
  return (
    <div className="group card overflow-hidden">
      {/* Product Image with Link */}
      <Link href={`/productos/${product.id}`} className="block relative h-40 sm:h-48 md:h-64 overflow-hidden">
        <img
          src={product.image}
          alt={product.name}
        ...
        {/* Action Buttons */}
        <div className="absolute top-1.5 sm:top-3 right-1.5 sm:right-3 flex flex-col gap-1 sm:gap-2">
          <button
            onClick={(e) => {
              e.preventDefault();
              toggleFavorite();
        ...
        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
          <div className="bg-white text-primary-600 px-2 sm:px-4 py-1.5 sm:py-2 rounded-lg font-medium flex items-center gap-1 sm:gap-2">
            <Eye size={14} className="sm:w-4 sm:h-4" />
            <span className="text-xs sm:text-sm">Ver Producto</span>
          </div>
        </div>
      </Link>
      ...
      <div className="p-2 sm:p-4">
        <div className="flex items-center justify-between mb-1 sm:mb-2">
          <span className="text-xs text-muted uppercase tracking-wide truncate">
            {product.category}
          </span>
          <div className="flex items-center">
            <Star size={12} className="text-yellow-400 fill-current sm:w-3.5 sm:h-3.5" />
            <span className="text-xs sm:text-sm text-body ml-1">
        ...
        <Link href={`/productos/${product.id}`} className="block">
          <h3 className="font-semibold text-sm sm:text-base text-title mb-1 sm:mb-2 line-clamp-2 hover:text-primary-600 transition-colors">
            {product.name}
          </h3>
        </Link>
