...
export class EnvioClickClient {
  ...
  private transformPayloadToEnvioClick(payload: ShippingPayload): Record<string, unknown> {
    return {
      ...
      origin: {
        company_origin: payload.address_from?.company || payload.addressFrom?.company || "Mi Empresa",
        street_origin: payload.address_from?.street || payload.addressFrom?.street || "",
        interior_number_origin: payload.address_from?.interior_number || payload.addressFrom?.interior_number || "",
        outdoor_number_origin: payload.address_from?.number || payload.addressFrom?.number || "",
        zip_code_origin: payload.address_from?.postal_code || payload.addressFrom?.postalCode || "",
        neighborhood_origin: payload.address_from?.district || payload.addressFrom?.district || "",
        city_origin: payload.address_from?.city || payload.addressFrom?.city || "",
        state_origin: payload.address_from?.state || payload.addressFrom?.state || "",
        references_origin: payload.address_from?.references || payload.addressFrom?.references || "",
        name_origin: payload.address_from?.name || payload.addressFrom?.name || "",
        email_origin: payload.address_from?.email || payload.addressFrom?.email || "",
        phone_origin: payload.address_from?.phone || payload.addressFrom?.phone || "",
        save_origin: "false"
      },
      destination: {
