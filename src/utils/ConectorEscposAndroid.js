const ConectorEscposAndroid = (() => {
    ...
    class ConectorPlugin {
        ...
        // Método estático para probar conexión
        static async testConnection(ruta, phoneIp) {
            try {
                const baseUrl = phoneIp ? `http://${phoneIp}:8000` : (ruta || ConectorPlugin.URL_PLUGIN_POR_DEFECTO);
                const response = await fetch(baseUrl + "/impresoras");
                return response.ok;
            } catch (error) {
                return false;
            }
        ...
        // Método estático para obtener impresoras con IP personalizada
        static async obtenerImpresorasConIp(phoneIp) {
            try {
                const baseUrl = phoneIp ? `http://${phoneIp}:8000` : ConectorPlugin.URL_PLUGIN_POR_DEFECTO;
                const response = await fetch(baseUrl + "/impresoras");
                return await response.json();
            } catch (error) {
                return [];
